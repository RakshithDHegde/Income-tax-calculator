...
function displayResult() {
  ...
  exemptions_80c = Math.min(
    150000,
    AutoNumeric.getNumber("#exemptions-80c") + pf
  );
  ...
  $("#old-taxable-income-monthly").text(
    formatCurrencyMonthly(oldTaxableIncome)
  );
  $("#new-taxable-income-annual").text(formatCurrency(newTaxableIncome));
  $("#new-taxable-income-monthly").text(
    formatCurrencyMonthly(newTaxableIncome)
  );
  ...
  $("#standard-exemption-monthly").text(
    formatCurrencyMonthly(STANDARD_EXEMPTION)
  );
  ...
  $("#nr-standard-exemption-monthly").text(
    formatCurrencyMonthly(STANDARD_EXEMPTION)
  );
  ...
  oldTakeHome = Math.round(
    income - pf - EMPLOYER_PF - PROFESSIONAL_TAX - oldTax - oldTaxCess
  );
  newTakeHome = Math.round(
    income - pf - EMPLOYER_PF - PROFESSIONAL_TAX - newTax - newTaxCess
  );
  ...
  $("#or-home-professional-tax-monthly").text(
    formatCurrencyMonthly(PROFESSIONAL_TAX)
  );
  ...
  $("#nr-home-professional-tax-monthly").text(
    formatCurrencyMonthly(PROFESSIONAL_TAX)
  );
  ...
  if (oldTakeHome > newTakeHome) {
    ...
    $("#result").text(
      `Old regime is better than New regime by ${takeHomeDifference} per month`
    );
  } else {
    ...
    $("#result").text(
      `New regime is better than Old regime by ${takeHomeDifference} per month`
    );
  }
}
// Jira API endpoint URLs
const apiUrl = "https://user123.atlassian.net/rest/api/2";
const agileApiUrl = "https://user123.atlassian.net/rest/agile/1.0";

// Jira credentials (replace with your actual credentials)
const username = "keer2809@gmail.com";
const password =
...
// Custom field IDs (replace with actual custom field IDs)
const investmentAmountCustomFieldId = "customfield_12345";
const investmentTypeCustomFieldId = "customfield_67890";

// Function to create an issue and move it to the "users" column

async function createAndMoveIssue() {
  alert("A");
  try {
    ...
    var raw = JSON.stringify({
      "Annual-Income": Number(
        document.getElementById("total-income").value.substring(1)
      ),
      "Employee-PF": Number(document.getElementById("pf").value.substring(1)),
      "House-Rent-Allowance": Number(
        document.getElementById("hra").value.substring(1)
      ),
      "Exemptions-80C": Number(
        document.getElementById("exemptions-80c").value.substring(1)
      ),
      "Exemptions-MISC": Number(
        document.getElementById("exemptions-others").value.substring(1)
      ),
    });
...
function calculateTax(taxableIncome, old = false) {
  if (old) {
    ...
    slab5Tax = slab5Income * 0.2;
    slab6Income = Math.max(taxableIncome - 1500000, 0);
    slab6Tax = slab6Income * 0.3;
